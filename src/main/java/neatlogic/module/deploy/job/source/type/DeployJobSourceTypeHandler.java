...
public class DeployJobSourceTypeHandler extends AutoexecJobSourceTypeHandlerBase {
    ...
    public void checkinSqlList(JSONObject paramObj) {
        autoexecJobMapper.getJobLockByJobId(paramObj.getLong("jobId"));
        List<DeploySqlNodeDetailVo> insertSqlDetailList = new ArrayList<>();
        List<DeploySqlNodeDetailVo> updateSqlDetailList = new ArrayList<>();
        List<Long> needDeleteSqlIdList = new ArrayList<>();
        Map<String, DeploySqlNodeDetailVo> oldJobPhaseAndSqlDetailMap = new HashMap<>();
        Map<String, DeploySqlNodeDetailVo> oldSqlDetailMap = new HashMap<>();
        List<Long> insertJobPhaseSqlIdList = new ArrayList<>();
        //是否需要更新阶段&node为pending
        boolean isNeedUpdatePending = false;
        ...
        List<DeploySqlNodeDetailVo> oldSqlDetailList = deploySqlMapper.getAllDeploySqlDetailListWithJob(new DeploySqlNodeDetailVo(paramObj.getLong("sysId"), paramObj.getLong("moduleId"), paramObj.getLong("envId"), paramObj.getString("version"), paramObj.getString("targetPhaseName")));
        if (CollectionUtils.isNotEmpty(oldSqlDetailList)) {
            ...
            for (DeploySqlNodeDetailVo detailVo : oldSqlDetailList) {
                oldSqlDetailMap.putIfAbsent(detailVo.getResourceId().toString() + detailVo.getSqlFile(), detailVo);
            }
            //默认删除所有老sql，后续再跟paramSql对比判断再去掉需要执行的sql
            needDeleteSqlIdList = oldSqlDetailList.stream().map(DeploySqlNodeDetailVo::getId).collect(Collectors.toList());
        }

        if (CollectionUtils.isNotEmpty(paramSqlVoArray)) {
            List<DeploySqlNodeDetailVo> paramSqlDetailList = paramSqlVoArray.toJavaList(DeploySqlNodeDetailVo.class);
            for (int i = 0; i < paramSqlDetailList.size(); i++) {
                ...
                DeploySqlNodeDetailVo oldSqlDetailVo = oldJobPhaseAndSqlDetailMap.get(jobId.toString() + targetPhaseVo.getName() + paramSqlDetailVo.getResourceId().toString() + paramSqlDetailVo.getSqlFile());
                //作业阶段不存在则新增
                if (oldSqlDetailVo == null) {
                    DeploySqlNodeDetailVo deploySqlDetailVo = oldSqlDetailMap.get(paramSqlDetailVo.getResourceId().toString() + paramSqlDetailVo.getSqlFile());
                    if (deploySqlDetailVo != null) {
                        paramSqlDetailVo.setId(deploySqlDetailVo.getId());
                        updateSqlDetailList.add(paramSqlDetailVo);
                        insertJobPhaseSqlIdList.add(paramSqlDetailVo.getId());
                    } else {
                        insertSqlDetailList.add(paramSqlDetailVo);
                    }
                } else {
                    if (CollectionUtils.isNotEmpty(needDeleteSqlIdList)) {
                        //旧数据 - 需要更新的数据 = 需要删除的数据
                        needDeleteSqlIdList.remove(oldSqlDetailVo.getId());
                    }
                    paramSqlDetailVo.setId(oldSqlDetailVo.getId());
                    updateSqlDetailList.add(paramSqlDetailVo);
                }
            }
        ...
        if (CollectionUtils.isNotEmpty(insertJobPhaseSqlIdList)) {
            for (Long sqlId : insertJobPhaseSqlIdList) {
        ...
        if (CollectionUtils.isNotEmpty(updateSqlDetailList)) {
            for (DeploySqlNodeDetailVo sqlDetailVo : updateSqlDetailList) {
                deploySqlMapper.updateDeploySqlDetail(sqlDetailVo);
            }
        }

        //存在新增或修改过的sql，需要重置当前阶段的状态
        if (isNeedUpdatePending) {
    ...
    public void addExtraJobPhaseNodeInfoByList(Long jobId, List<AutoexecJobPhaseNodeVo> jobPhaseNodeVoList) {
        if (jobId != null && CollectionUtils.isNotEmpty(jobPhaseNodeVoList)) {
            ...
            if (deployJobVo != null) {
                ...
                List<DeployJobResourceBlueGreenVo> deployResourceBlueGreenVos = deployBlueGreenMapper.listDeployJobResourceBlueGreen(deployJobVo.getId());
                Map<Long, DeployJobResourceBlueGreenVo> deployJobResourceBlueGreenVoMap = new HashMap<>();
                if (CollectionUtils.isNotEmpty(deployResourceBlueGreenVos)) {
                    deployJobResourceBlueGreenVoMap = deployResourceBlueGreenVos.stream().collect(Collectors.toMap(DeployJobResourceBlueGreenVo::getResourceId, e -> e));
                }
                if (CollectionUtils.isNotEmpty(instanceIdList)) {
                    ...
                    for (AutoexecJobPhaseNodeVo jobPhaseNodeVo : jobPhaseNodeVoList) {
                        ...
                        DeployJobResourceBlueGreenVo deployInstanceBlueGreenVo = deployJobResourceBlueGreenVoMap.get(jobPhaseNodeVo.getResourceId());
    ...
    public void addExtraJobPhaseNodeBlueGreenInfoByList(Long jobId, List<AutoexecJobPhaseNodeVo> jobPhaseNodeVoList) {
        if (jobId != null && CollectionUtils.isNotEmpty(jobPhaseNodeVoList)) {
            DeployJobVo deployJobVo = deployJobMapper.getDeployJobByJobId(jobId);
            if (deployJobVo != null) {
                List<Long> jobResourceIdList = jobPhaseNodeVoList.stream().map(AutoexecJobPhaseNodeVo::getResourceId).filter(Objects::nonNull).collect(Collectors.toList());
                List<DeployJobResourceBlueGreenVo> deployJobResourceBlueGreenVos = deployBlueGreenMapper.listDeployJobResourceBlueGreen(deployJobVo.getId());
                Map<Long, DeployJobResourceBlueGreenVo> deployJobResourceBlueGreenVoMap = new HashMap<>();
                if (CollectionUtils.isNotEmpty(deployJobResourceBlueGreenVos)) {
                    deployJobResourceBlueGreenVoMap = deployJobResourceBlueGreenVos.stream().collect(Collectors.toMap(DeployJobResourceBlueGreenVo::getResourceId, e -> e));
                }
                if (CollectionUtils.isNotEmpty(jobResourceIdList)) {
                    JSONObject extraInfo = null;
                    for (AutoexecJobPhaseNodeVo jobPhaseNodeVo : jobPhaseNodeVoList) {
                        extraInfo = jobPhaseNodeVo.getExtraInfo();
                        if (extraInfo == null) {
                        ...
                        DeployJobResourceBlueGreenVo deployJobResourceBlueGreenVo = deployJobResourceBlueGreenVoMap.get(jobPhaseNodeVo.getResourceId());
                        if (deployJobResourceBlueGreenVo != null) {
                            extraInfo.put("blueGreenId", deployJobResourceBlueGreenVo.getBlueGreenId());
                            extraInfo.put("blueGreenName", deployJobResourceBlueGreenVo.getBlueGreenName());
                            extraInfo.put("blueGreenSort", deployJobResourceBlueGreenVo.getBlueGreenSort());
                        }
                        jobPhaseNodeVo.setExtraInfo(extraInfo);
                    }
                }
    ...
    @Override
    public void handleAddJobPhaseNodeEvent(AutoexecJobVo jobVo, List<AutoexecJobPhaseNodeVo> autoexecJobPhaseNodeVos, String userName, Long protocolId, Long updateTag) {
        //补充作业节点蓝绿关系
        if (CollectionUtils.isNotEmpty(autoexecJobPhaseNodeVos)) {
            ...
            if (CollectionUtils.isNotEmpty(deployInstanceBlueGreenVos)) {
                ...
                List<DeployJobResourceBlueGreenVo> deployJobResourceBlueGreenVos = new ArrayList<>();
                for (AutoexecJobPhaseNodeVo autoexecJobPhaseNodeVo : autoexecJobPhaseNodeVos) {
                    DeployJobResourceBlueGreenVo blueGreenVo = new DeployJobResourceBlueGreenVo();
                    blueGreenVo.setJobId(jobVo.getId());
                    blueGreenVo.setResourceId(autoexecJobPhaseNodeVo.getResourceId());
