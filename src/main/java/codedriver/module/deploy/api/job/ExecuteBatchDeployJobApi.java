...
public class ExecuteBatchDeployJobApi extends PrivateApiComponentBase {
    ...
    @Input({
            ...
            @Param(name = "batchJobType", type = ApiParamType.LONG, desc = "批量作业执行策略，refire：跳过所有已完成的子作业；reset_refire:执行所有子作业"),
            @Param(name = "jobType", type = ApiParamType.LONG, desc = "子作业执行策略，refire：跳过所有已完成、已忽略的节点；reset_refire:执行所有节点"),
    })
    @Description(desc = "执行批量发布作业接口")
    @ResubmitInterval(value = 2)
    @Override
    public Object myDoService(JSONObject jsonObj) throws Exception {
        deployBatchJobService.fireBatch(jsonObj.getLong("id"));
        return null;
    }
